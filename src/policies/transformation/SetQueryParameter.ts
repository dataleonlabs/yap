...
@YapPolicy({
    ...
    description: "The set-query-parameter policy adds, replaces value of, "
    + "or deletes request query string parameter. Can be used to pass query parameters expected",
            ...
            for (const element of policyElement.elements) {
                ...
                if (!name) {
                    ...
                    if (!element.elements || !element.elements.length) {
                        errors.push(`${this.id}-ERR-003: At least one of value of query parameter `
                        + `should be set for node <${element.name}>. Example <value>Bearer error="invalid_token"</value>`);
                    }
                }
            }
        }
