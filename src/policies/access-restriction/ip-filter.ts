                ...
                switch (element.name) {
                    ...
                    case "address-range":
                        ...
                        if(!from || !to || !isIP(from) || !isIP(to)) {
                            errors.push(`${this.id}-ERR-006: Element name <${element.name}>. Have wrong attributes defined: from = ${from}, to = ${to}. Element should have correct from and to IP addreses defined, f.e. <address-range from="13.66.140.128" to="13.66.255.143"></address-subnet>`);
                        }
                        break;
                    default:
                        errors.push(`${this.id}-ERR-003: Element name <${element.name}> is not allowed. should be defined at least one sub element of <address>, <address-subnet> or <address-range>`);
                        break;
                }
            }
        }
