...
export default class SendRequest extends Policy {
    ...
    public validate(policyElement: any) {
        ...
        const checkSubElements = (element: any) => {
            ...
            if (!policyInstance) {
                errors.push(`policies-ERR-005: XML tag <${policyElement.name}> contains unknown policy <${element.name}>. `
            ...
            for (const element of policyElement.elements) {
                switch (element.name) {
                    ...
                    case 'set-body':
                        ...
                        break;
                    default:
                        errors.push(`${this.id}-ERR-005: policy <${element.name}> not allowed here. `
                        + `Allowed only <set-url>, <set-method>, <set-header>, <set-body>`);
                        break;
                }
            }
